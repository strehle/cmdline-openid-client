...
func main() {
	flag.Usage = func() {
		fmt.Println("Usage: openid-client <command> <flags>\n" +
			...
			"      -logout_uri        Logout URI. Optional parameter used in authorization_code flow.\n" +
	...
	var promptParameter = flag.String("prompt", "", "OIDC prompt parameter")
	var maxAgeParameter = flag.String("max_age", "", "OIDC max_age parameter")
	...
	var logoutUri = flag.String("logout_uri", "", "Logout URI for authorization_code flow.")
	...
	if *command != "" {
		...
		if *logoutUri != "" {
			requestMap.Set("logout_uri", *logoutUri)
		}
		if *idpParameter != "" {
